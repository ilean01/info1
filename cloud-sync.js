(() => {
  ...
  const localState = () => normalState(parse(localStorage.getItem(KEY), {}));
  const stateTime = s => {
  ...
  function addStyles() {
    ...
    s.textContent = `
      #info1CloudBadge{position:fixed;right:16px;bottom:16px;z-index:9997;border:1px solid #475569;background:rgba(9,16,31,.97);color:#eef2ff;border-radius:15px;padding:10px 12px;box-shadow:0 14px 40px #0007;font:700 12px/1.25 system-ui;max-width:min(560px,calc(100vw - 32px));backdrop-filter:blur(12px)}
      ...
      #info1CloudBadge button{margin:5px 0 0 7px;border:0;border-radius:9px;padding:6px 9px;background:#334155;color:#fff;font-weight:800;cursor:pointer}
  ...
  function showAuth() {
    ...
    o.querySelector('#info1Offline').onclick = () => {
      localStorage.setItem(OFFLINE_KEY, '1'); o.remove();
  ...
  async function hydrateFirst() {
    ...
    if (knownRevision !== remoteRevision) {
      await installRemote(remote, remoteRevision, 'revision-mismatch-cloud-wins');
      return true;
    }

    const hasExplicitLocalChanges = localStorage.getItem(UNSYNCED_KEY) === '1';
    if (hasExplicitLocalChanges && stateTime(local) > stateTime(remote)) {
      dirty = true;
      hydrated = true;
      return false;
    }

    await installRemote(remote, remoteRevision, 'cloud-authoritative');
    return true;
  }

  function showSyncedBadge() {
    const name = workspace?.name || 'INFO 1';
    badge(`☁️ Sincronizado · ${name} · rev ${remoteRevision}`, 'ok', '<button id="info1PullCloud">Cargar nube</button><button id="info1ImportBackup">Importar backup</button><button id="info1Logout">Salir</button>');
    ...
    document.getElementById('info1ImportBackup').onclick = importBackup;
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        if (parsed?.localStorage) {
          ...
          if (!raw) throw new Error('El archivo no contiene el estado de INFO1.');
        ...
        await push();
        if (!dirty) {
          badge('✅ Backup restaurado en Supabase · recargando…', 'ok');
  ...
  async function push() {
    if (!dirty || busy || !session || !workspace || !hydrated) return;
    busy = true;
    dirty = false;
    try {
      const local = localState();
      ...
      const knownRevision = Number(localStorage.getItem(`${HYDRATED_PREFIX}${workspace.id}`) || 0);
      if (cur && rev !== knownRevision) {
        await installRemote(cloud, rev, 'remote-revision-changed-before-push');
        return;
      }

      if (progressScore(cloud) > 0 && progressScore(local) === 0) {
        await installRemote(cloud, rev, 'zero-progress-write-blocked');
        return;
      }
  ...
  sb.auth.onAuthStateChange((event, s) => {
    ...
    if (event === 'SIGNED_OUT') { session = null; workspace = null; hydrated = false; }
  });

  window.addEventListener('online', () => { if (session && workspace && hydrated) push(); });
  boot();
})();